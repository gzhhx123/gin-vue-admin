  ...
  <div>
    <div class="gva-search-box">
      ...
      <el-date-picker v-model="searchInfo.endCreatedAt" type="datetime" placeholder="结束日期" :disabled-date="time=> searchInfo.startCreatedAt ? time.getTime() < searchInfo.startCreatedAt.getTime() : false"></el-date-picker>
        ...
        <el-form-item label="卖家平台id" prop="sellerPlatformCode">
         <el-input v-model="searchInfo.sellerPlatformCode" placeholder="搜索条件" />
        </el-form-item>
    ...
    <div class="gva-table-box">
        <div class="gva-btn-list">
            ...
            <ExportTemplate  template-id="bagique_Seller" />
            <ExportExcel  template-id="bagique_Seller" />
            <ImportExcel  template-id="bagique_Seller" @on-success="getTableData" />
        </div>
        <el-table
        ref="multipleTable"
        style="width: 100%"
    ...
    <el-drawer destroy-on-close size="800" v-model="dialogFormVisible" :show-close="false" :before-close="closeDialog">
       <template #header>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            <el-form-item label="卖家名称:"  prop="sellerName" >
              <el-input v-model="formData.sellerName" :clearable="true"  placeholder="请输入卖家名称" />
            </el-form-item>
            <el-form-item label="卖家平台id:"  prop="sellerPlatformCode" >
              <el-input v-model="formData.sellerPlatformCode" :clearable="true"  placeholder="请输入卖家平台id" />
            </el-form-item>
            <el-form-item label="备注:"  prop="remark" >
    ...
    <el-drawer destroy-on-close size="800" v-model="detailShow" :show-close="true" :before-close="closeDetailShow" title="查看">
            <el-descriptions :column="1" border>
                    ...
                    <el-descriptions-item label="卖家平台id">
                        {{ detailFrom.sellerPlatformCode }}
                    </el-descriptions-item>
...
const getTableData = async() => {
  const table = await getSellerList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteSellerByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateSellerFunc = async(row) => {
    const res = await findSeller({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createSeller(formData.value)
                  break
                case 'update':
                  res = await updateSeller(formData.value)
                  break
                default:
                  res = await createSeller(formData.value)
                  break
