  ...
  <div>
    <div class="gva-search-box">
      ...
      <el-date-picker v-model="searchInfo.endCreatedAt" type="datetime" placeholder="结束日期" :disabled-date="time=> searchInfo.startCreatedAt ? time.getTime() < searchInfo.startCreatedAt.getTime() : false"></el-date-picker>
        ...
        <el-form-item label="公司简称" prop="companyShortName">
         <el-input v-model="searchInfo.companyShortName" placeholder="搜索条件" />
        </el-form-item>
        <el-form-item label="查询链接" prop="companyUrl">
         <el-input v-model="searchInfo.companyUrl" placeholder="搜索条件" />
        </el-form-item>
    ...
    <div class="gva-table-box">
        <div class="gva-btn-list">
            ...
            <ExportTemplate  template-id="bagique_TrackCompany" />
            <ExportExcel  template-id="bagique_TrackCompany" />
            <ImportExcel  template-id="bagique_TrackCompany" @on-success="getTableData" />
        </div>
        <el-table
        ref="multipleTable"
        style="width: 100%"
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateTrackCompanyFunc(scope.row)">编辑</el-button>
            <el-button  type="primary" link icon="delete" @click="deleteRow(scope.row)">{{scope.row.DeletedAt?'彻底删除':'删除'}}</el-button>
            <el-button  type="primary" link icon="refresh-left" @click="restoreRow(scope.row)" v-if="scope.row.DeletedAt">恢复</el-button>
            </template>
        </el-table-column>
        </el-table>
    ...
    <el-drawer destroy-on-close size="800" v-model="detailShow" :show-close="true" :before-close="closeDetailShow" title="查看">
            <el-descriptions :column="1" border>
                    ...
                    <el-descriptions-item label="查询链接">
                        {{ detailFrom.companyUrl }}
                    </el-descriptions-item>
...
<script setup>
  import {
    createTrackCompany,
    deleteTrackCompany,
    deleteTrackCompanyByIds,
    updateTrackCompany,
    findTrackCompany,
    getTrackCompanyList, restoreTrackCompany
...
const getTableData = async() => {
  const table = await getTrackCompanyList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteTrackCompanyByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateTrackCompanyFunc = async(row) => {
    const res = await findTrackCompany({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteTrackCompanyFunc = async (row) => {
  const res = await deleteTrackCompany({ ID: row.ID , TYPE: row.DeletedAt ? 'HARD' : 'SOFT' })
...
// 恢复行
const restoreTrackCompanyFunc = async (row) => {
  const res = await restoreTrackCompany({ ID: row.ID })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '恢复成功'
